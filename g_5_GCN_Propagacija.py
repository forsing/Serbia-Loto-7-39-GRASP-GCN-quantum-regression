...
# Varijanta: GCN embedding (1 sloj normalizovane propagacije, bez učenja težina)

"""
Graphs in Space: Graph Embeddings for Machine Learning on Complex Data
GCN = D^(-1/2) A_tilde D^(-1/2) @ H, jedan sloj
"""
...
Matrica susedstva A iz M (čvorovi 1..39), 
A_tilde = A + I, D_tilde = stepeni čvorova, 
A_norm = D_tilde^(-1/2) A_tilde D_tilde^(-1/2).

Početni H0: 
prvih k sopstvenih vektora Laplaciana, oblik (39, k), k = 3.

Jedan GCN sloj: H1 = A_norm @ H0 (bez učenih težina).

Rezultat se normalizuje u [0, π]. 
Samo numpy i qiskit.
"""
...
def compute_gcn_embeddings(M, k=EMBED_DIM):
    ...
    n_eig = min(k, vecs.shape[1] - 1)
    H0 = vecs[:, 1:1 + n_eig].astype(float)
    if n_eig < k:
        H0 = np.hstack([H0, np.tile(H0[:, -1:], (1, k - n_eig))])
    ...
    for d in range(k):
        ...
        if max_v - min_v > 0:
            H1[:, d] = (col - min_v) / (max_v - min_v) * np.pi
        else:
            H1[:, d] = 0.0
    return H1
...
def main():
    ...
    emb = compute_gcn_embeddings(M, k=EMBED_DIM)
    ...
    print()
    print("Predikcija (GCN + kvantna regresija, deterministički, strukturno):")
    print(best_combo)
    print()
    """
    Predikcija (GCN + kvantna regresija, deterministički, strukturno):
    (7, 8, x, y, z, 33, 34)
    """
